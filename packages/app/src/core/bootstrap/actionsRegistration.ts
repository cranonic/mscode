...
/**
 * Format the entire active document (uses LSP DocumentFormatting provider when available).
 */
...
/** Format only the current selection. */
commands.registerCommand(
...
/**
 * Rename the symbol under the cursor (uses LSP Rename provider + Monaco rename widget).
 * This is different from workbench.action.renameActiveFile (file rename modal).
 */
commands.registerCommand(
...
/** Go to definition under cursor (LSP Definition provider). */
commands.registerCommand(
