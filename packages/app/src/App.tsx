...
import { initNativeStatusBar, watchThemeStatusBar, notifyThemeChanged } from '@/core/native/statusBarSync';
...
const App = () => {
  ...
  useEffect(() => {
    ...
    const initializeApp = async () => {
      ...
      // Android system status bar ↔ theme (--ms-statusbar-bg / --ms-bg-side)
      initNativeStatusBar();
      try {
        const disposeWatch = watchThemeStatusBar();
        let lastTheme = useSettingsStore.getState().settings['workbench.theme'];
        const unsubSettings = useSettingsStore.subscribe((state) => {
          const next = state.settings['workbench.theme'];
          if (next !== lastTheme) {
            lastTheme = next;
            setTimeout(() => notifyThemeChanged(), 80);
            setTimeout(() => notifyThemeChanged(), 250);
          }
        });
        (window as any).__msStatusBarDispose = () => {
          disposeWatch();
          unsubSettings();
        };
      } catch (e) {
