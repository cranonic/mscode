...
export const MainLayout: React.FC<{ children?: React.ReactNode }> = ({ children }) => {
  ...
  const { isOpen, isClosing, position, origin, items, closeMenu } = useMenuStore();
  ...
  // and crashes the IDE on tab switch / tab-bar reorder.
  const editorMountOrderRef = useRef<string[]>([]);
  {
    const openIds = new Set(tabs.map(t => t.id));
    // Drop closed tabs (keep relative order of survivors)
    editorMountOrderRef.current = editorMountOrderRef.current.filter(id => openIds.has(id));
    // Append newly opened tabs at the end only
    for (const t of tabs) {
      if (!editorMountOrderRef.current.includes(t.id)) {
        editorMountOrderRef.current.push(t.id);
      }
    }
  }
  ...
  useEffect(() => {
    ...
    // Push status-bar colors after theme CSS vars settle
    notifyThemeChanged();
  ...
  return (
    <div className="ms-master-container" style={{ position: 'relative', display: 'flex', flexDirection: 'column', height: '100dvh', width: '100vw', overflow: 'hidden' }}>
      ...
      <div className="ms-editor-layout" onClick={closeMenu} style={{ position: 'relative' }}>
        ...
        <div className="content-area" style={{ position: 'relative', zIndex: 1 }} onClick={handleEditorClick}>
          ...
          <article className="editor-view">
            {tabs.length > 0 ? (
              // Iterate in FIXED mount order (append-only). Tab-bar order / LRU
              // must never reshuffle these children — Monaco cannot survive DOM moves.
              (() => {
                const tabById = new Map(tabs.map(t => [t.id, t]));
                return editorMountOrderRef.current.map(id => {
                  const tab = tabById.get(id);
                  if (!tab) {
                    // Keep a keyed empty slot so later siblings never shift in the DOM
                    return <div key={id} style={{ display: 'none' }} />;
                  }

                  // LRU: outside cache → empty placeholder (same key, no Monaco).
                  // This prevents sibling editor DOM nodes from moving when a middle tab is evicted.
                  if (!mountedTabIds.has(id)) {
                  ...
                  return (
                    <div key={tab.id} style={{ display: activeTabId === tab.id ? 'block' : 'none', height: '100%', width: '100%' }}>
                      ...
                      {(() => {
                        ...
                        // LAYER 1.5: Custom Previewer Registry (Images, PDFs, etc.)
                        if (tab.type !== 'fallback_text') {
                          const CustomPreviewer = customPreviewerRegistry.getPreviewerForExtension(tab.filePath || tab.id);
                          if (CustomPreviewer) {
                            const PreviewerComponent = CustomPreviewer.component;
                            return <PreviewerComponent tabId={tab.id} filePath={tab.filePath || tab.id} />;
                          }
                        }

                        // LAYER 2: Extension API & Core Registry (The Standard Way)
                        const resolveType = tab.type === 'fallback_text' ? 'code' : tab.type;
                        const RegisteredCustomView = tabRegistry.getTab(resolveType);
                        ...
                        if (RegisteredCustomView) {
                          return (
                             <RegisteredCustomView 
                               ...
                               tab={tab}
                               extensionId={tab.filePath} 
                               mode={resolveType === 'termis' ? 'fullscreen' : undefined} 
                             />
                          );
                        }
      ...
      <StatusBar />
      {(isOpen || isClosing) && items.length > 0 && (
        <ContextMenu
          items={items}
          isClosing={isClosing}
          origin={origin}
          style={{ position: 'fixed', top: position.y, left: position.x, zIndex: 999999 }}
        />
      )}
      <CommandPalette />
    </div>
