...
export interface ContextMenuProps {
   ...
   * Defaults to `'right'` → `transformOrigin: 'top left'`.
   */
  openSide?: 'right' | 'left';
}
...
export const ContextMenu: React.FC<ContextMenuProps> = ({ items, style, isNested, isClosing, openSide = 'right' }) => {
  ...
  // click meant for a menu item, so the menu looks like it opens then instantly closes.
  // Nested submenus get 'relative' from their parent via the `style` prop, which wins
  // here because it's spread after the default.
  const [adjustedStyle, setAdjustedStyle] = useState<React.CSSProperties>(
    isNested ? (style || {}) : { position: 'fixed', ...style }
  );
  
  const [activeSubId, setActiveSubId] = useState<string | null>(null);
  const [subPosition, setSubPosition] = useState<'right' | 'left'>('right');

  // Derives the CSS transform-origin anchor so the scale animation grows from
  // the edge that is visually connected to its parent item/cursor.
  ...
  // browser paints, so the corrected position is applied in the same frame as
  // the initial render — eliminating the jump/blink caused by useEffect.
  useLayoutEffect(() => {
    if (menuRef.current) {
      ...
      if (!isNested && style) {
        ...
        setAdjustedStyle({ position: 'fixed', ...style, left: newLeft, top: newTop });
      } else if (isNested) {
