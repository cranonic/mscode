...
export class XtermAdapter {
  ...
  async init(): Promise<void> {
    ...
    this.xterm = new TerminalClass({
      ...
      convertEol:            true,
      disableStdin:          false,
      // Match Termux: long lines wrap to next row instead of horizontal scroll
      windowsMode:           false,
    ...
    // Fit then sync PTY size + force DEC auto-wrap (DECAWM) on
    setTimeout(() => {
      this._fitAndSyncPty();
      this._ensureAutowrap();
    }, 50);
  ...
  /** Enable DEC auto-wrap mode so lines wrap like Termux (no '<' overflow marker). */
  private _ensureAutowrap(): void {
    try {
      // CSI ? 7 h = DECAWM on (auto-wrap)
      this.xterm?.write('\x1b[?7h');
    } catch {}
  }

  private _fitAndSyncPty(): void {
    try {
      this.fitAddon?.fit();
