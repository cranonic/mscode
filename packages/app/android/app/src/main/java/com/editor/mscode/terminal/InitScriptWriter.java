...
public class InitScriptWriter {
    ...
    public void write(String outputPath, String projectCwd) throws IOException {
        ...
        sb.append("  export MSCODE_LINKER=\n");
        sb.append("fi\n");
        // proot from nativeLibraryDir — compilers need real execve (clang -cc1)
        ...
        sb.append("mscode_wrap\n");
        sb.append("\n");
        // ── Compilers via proot — user types: clang hello.c -o hello ─────────
        sb.append("# proot makes normal clang/gcc commands work (targetSdk>28)\n");
        sb.append("_mscode_proot() {\n");
        sb.append("  if [ -z \"$MSCODE_PROOT\" ] || [ ! -f \"$MSCODE_PROOT\" ]; then\n");
        sb.append("    echo \"mscode: libproot.so missing\" >&2; return 127\n");
        sb.append("  fi\n");
        ...
        sb.append("    -w \"$PWD\" \"$@\"\n");
        sb.append("}\n");
        // Override elf-wrappers for toolchains (must be AFTER mscode_wrap).
        // Names with + or . cannot be shell functions — use alias → helper.
        sb.append("_mscode_cc() {\n");
        sb.append("  _cc=\"$1\"; shift\n");
        sb.append("  if [ -f \"$PREFIX/bin/$_cc\" ]; then\n");
        sb.append("    _mscode_proot \"$PREFIX/bin/$_cc\" \"$@\"\n");
        sb.append("  else\n");
        sb.append("    echo \"$_cc: not installed — pkg install clang\" >&2\n");
        sb.append("    return 127\n");
        sb.append("  fi\n");
        sb.append("}\n");
        // valid function names
        for (String cc : new String[]{"clang", "gcc", "cc", "tcc", "lld"}) {
            sb.append(cc).append("() { _mscode_cc ").append(cc).append(" \"$@\"; }\n");
        }
        // alias for names with + or .
        sb.append("alias 'clang++'='_mscode_cc clang++'\n");
        sb.append("alias 'g++'='_mscode_cc g++'\n");
        sb.append("alias 'c++'='_mscode_cc c++'\n");
        sb.append("alias 'ld.lld'='_mscode_cc ld.lld'\n");
        // run ./binary — sdcard/tmp are noexec; proot loads the ELF
        sb.append("run() {\n");
        sb.append("  if [ $# -lt 1 ]; then echo \"usage: run <elf> [args…]\" >&2; return 1; fi\n");
        sb.append("  _rb=\"$1\"; shift\n");
        sb.append("  case \"$_rb\" in ./*|../*|/*) ;; *) _rb=\"./$_rb\" ;; esac\n");
        sb.append("  if [ ! -f \"$_rb\" ]; then echo \"run: not found: $_rb\" >&2; return 127; fi\n");
        sb.append("  _mscode_proot \"$_rb\" \"$@\"\n");
        sb.append("}\n");
        sb.append("\n");
        // Write BASH_ENV file so bash scripts get the same wrappers
        sb.append("_mscode_write_bash_env() {\n");
