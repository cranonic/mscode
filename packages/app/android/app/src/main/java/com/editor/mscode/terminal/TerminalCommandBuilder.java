...
public class TerminalCommandBuilder {
    ...
    public Map<String, String> buildBackgroundEnvMap() {
        ...
        // ── LD_PRELOAD: node-exec fix (targetSdk>28) ────────────────────────
        // $PREFIX/bin/node lives under app-private writable storage, so a
        // direct execve() of it is denied by the OS ("Permission denied")
        // regardless of file permission bits. libtermux-exec.so hooks
        // execve()/posix_spawn() inside the CALLING process to route around
        // this (memfd + fexecve trick), exactly like real Termux.
        //
        // CRITICAL: this must be set here, in the env map handed to
        // ProcessBuilder BEFORE `sh` is spawned — the dynamic linker only
        // reads LD_PRELOAD when a process image is loaded. Exporting it
        // later, inside a running shell script, is too late: that shell's
        // own libc symbols are already resolved and won't be hooked.
        File termuxExec = new File(nativeLibDir, "libtermux-exec.so");
        if (termuxExec.isFile()) {
            map.put("LD_PRELOAD", termuxExec.getAbsolutePath());
        }
