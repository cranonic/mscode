...
import { runScaffolder } from '../lib/generator.js';
...
const projectCommands = ['build', 'watch', 'package', 'publish', 'release'];
...
if (projectCommands.includes(command)) {
  ...
  // Forward the command and arguments to the local script
  const child = spawn('node', [localScript, ...args], { stdio: 'inherit' });
  child.on('exit', code => process.exit(code));
} 
// ─── GENERATOR: If no command, run scaffolder ───
else {
