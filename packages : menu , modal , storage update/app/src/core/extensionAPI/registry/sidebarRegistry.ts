...
class SidebarRegistry {
  ...
  registerPanel(panel: SidebarPanelDef): () => void {
    const existingPanel = this.panels.get(panel.activityBarId);
    
    if (existingPanel && existingPanel.sections) {
      const injectedSections = existingPanel.sections.filter(s => (s as any)._injected);

      panel.sections = [...(panel.sections || []), ...injectedSections];
      
      if (!panel.header && existingPanel.header) {
          panel.header = existingPanel.header;
      }
  ...
  addSection(activityBarId: string, section: SidebarSectionDef): () => void {
    ...
    const injectedSection = { ...section, _injected: true } as any;

    const idx = panel.sections.findIndex(s => s.id === section.id);
    if (idx >= 0) panel.sections[idx] = injectedSection;
    else          panel.sections = [...panel.sections, injectedSection];
